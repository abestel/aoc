...
object Part1 extends IOApp.Simple {
  ...
  implicit val commandable: Commandable[Submarine] =
    Commandable.instance(Submarine()) {
      case Command.Forward(delta) => t => t.copy(x = t.x + delta)
      case Command.Up(delta)      => t => t.copy(y = t.y - delta)
      case Command.Down(delta)    => t => t.copy(y = t.y + delta)
    }

  override def run: IO[Unit] =
    stream[IO, Submarine]
      .map { case Submarine(x, y) => x * y }
